...
- Consumed as the `boot/` **source pin** by the assembly (`mica-build`) and by every board repository (`mica-x64`, `mica-virt-arm64`, `mica-cx3576`, `mica-s905x5m`): `deps/sources/mica-boot.json` there names a commit and the release asset `mica-boot-<commit12>.tar.gz`; every script derives its consumer as the directory above `boot/`, or this checkout when it carries its own `build-env/`
- A change here is released by `make publish-source` (or the workflow) as `build-<commit12>` and pinned in each consumer with `bash tools/deps.sh bump mica-boot`
