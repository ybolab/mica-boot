...
bash dev-keys.sh --out "$work/keys"
...
if bash dev-keys.sh --out "$work/keys" > "$work/refusal.log" 2>&1; then
...
if bash dev-keys.sh --out "$work/alias" > "$work/alias-refusal.log" 2>&1; then
    echo 'FAIL: generator accepted an existing output alias' >&2; exit 1
fi
bash init-keys.sh --out "$work/initialized"
snapshot() {
...
mkdir "$work/empty"
bash init-keys.sh --out "$work/empty" > "$work/concurrent-a.log" 2>&1 &
first=$!
bash init-keys.sh --out "$work/empty" > "$work/concurrent-b.log" 2>&1 &
second=$!
wait "$first"
wait "$second"
bash init-keys.sh --out "$work/empty"
for fault in incomplete mismatch symlink permissions; do
    ...
    if bash init-keys.sh --out "$work/$fault" > "$work/$fault.log" 2>&1; then
        echo "FAIL: initializer accepted $fault" >&2; exit 1
    fi
    snapshot "$work/$fault" > "$work/$fault-after"
    cmp "$work/$fault-before" "$work/$fault-after"
done
if bash init-keys.sh --out "$work/alias" > "$work/init-alias.log" 2>&1; then
