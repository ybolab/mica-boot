...
for args, extra, target in cases:
    record.write_text('')
    result = subprocess.run(['bash', str(repo / 'build-tools.sh'), *args],
    ...
    if not target:
        ...
        argv = calls[0]
        assert argv[0] == 'build' and argv[-1] == str(repo)
...
recipe = (repo / 'Dockerfile').read_text().replace('\\\n', '')
...
for target in ('x64', 'aa64'):
    ...
    (root / 'versions.env').write_bytes((repo / 'versions.env').read_bytes())
...
bash "$REPO/initramfs.sh" "$WORK/first" x64
...
bash "$REPO/initramfs.sh" "$WORK/repeat" x64
